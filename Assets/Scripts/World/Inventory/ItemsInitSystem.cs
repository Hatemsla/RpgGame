...
namespace World.Inventory
{
    public class ItemsInitSystem : IEcsInitSystem
    {
        ...
        public void Init(IEcsSystems systems)
        {
            foreach (var loadDataEventEntity in _loadDataFilter.Value)
            {
                ...
                foreach (var playerEntity in _playerFilter.Value)
                {
                    ...
                    var items = loadDataEventComp.IsLoadData
                        ? _cf.Value.inventoryConfiguration.allItems
                        : _cf.Value.inventoryConfiguration.items;
                    ...
                    for (var i = 0; i < items.Count; i++)
                    {
                        if (loadDataEventComp.IsLoadData)
                        {
                            ...
                            foreach (var loadItem in loadDataEventComp.PlayerSaveData.ItemDatas.Items)
                            {
                                if (itemData.itemName == loadItem.ItemName)
                                {
                                    weight = CreateItem(items, i, weight, playerInventoryViewContent, playerEntity, playerComp, hasItems);
                                }
                            }
                        }
                        else
        ...
        private float CreateItem(List<ItemData> items, int i, float weight, ContentView playerInventoryViewContent, int playerEntity,
            PlayerComp playerComp, HasItems hasItems)
        {
            var itemData = items[i];
            ...
            var itemView = Object.Instantiate(itemData.itemViewPrefab, Vector3.zero,
                Quaternion.identity);
