        ...
        foreach (var entity in _unitsMove.Value)
        {
                ...
                currentHorizontalSpeed > targetSpeed + speedOffset)
            {
                _speed = Mathf.Lerp(currentHorizontalSpeed, targetSpeed * inputMagnitude,
                    _ts.Value.DeltaTime * _cf.Value.speedChangeRate);
            ...
            if (input.Move != Vector2.zero)
            {
                ...
                var rotation = Mathf.SmoothDampAngle(player.Transform.eulerAngles.y, _targetRotation, ref _rotationVelocity,
                    _cf.Value.rotationSmoothTime);
